...
export async function cliOnboard(opts: OnboardOptions): Promise<void> {
  ...
  if (endpointArg) {
    if (!ENDPOINT_TYPES.includes(endpointArg as EndpointType)) {
      logger.error(
        `Invalid endpoint type: ${endpointArg}. Must be one of: ${ENDPOINT_TYPES.join(", ")}`,
      );
      return;
    }
    const ep = endpointArg as EndpointType;
  ...
  switch (endpointType) {
    ...
    case "ncp":
      ...
      if (!ncpPartner) {
        ...
        ncpPartner = await promptInput("NCP partner name");
      }
      endpointUrl = urlArg || "";
      if (!endpointUrl) {
        if (nonInteractive) {
        ...
        endpointUrl = await promptInput("NCP endpoint URL (e.g., https://partner.api.nvidia.com/v1)");
      }
      break;
    case "nim-local":
      endpointUrl = urlArg || "";
      if (!endpointUrl) {
        ...
        endpointUrl = await promptInput("NIM endpoint URL", "http://nim-service.local:8000/v1");
      }
      break;
    case "vllm":
    ...
    case "custom":
      ...
      if (!endpointUrl) {
        ...
        endpointUrl = await promptInput("Custom endpoint URL");
      }
      break;
  }
